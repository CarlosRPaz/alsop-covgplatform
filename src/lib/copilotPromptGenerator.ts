...
4. MANUAL ATTACHMENTS (NO DOWNLOAD LINKS): State that the Coverage Report and Replacement Cost Estimate (RCE) are attached directly to the email. Do NOT generate file URLs or download links.

5. CALENDLY / MEETING LINK: When provided, include the Calendly scheduling link so the client can book a review with an agent online.

6. CLIENT RESPONSIBILITY DISCLAIMER: You must include this notice near the end of the email:
   "Please remember that final coverage selections and decisions remain the responsibility of the policyholder."

7. CONCISENESS & RESPECT FOR TIME: Keep the entire email concise (under 200 words). Reassure the client that an annual review takes only a few minutes.

8. OFFICE PHONE & CONTACT: Direct the client to call our main office at (909) 626-5000 to speak with a licensed agent or schedule online via Calendly.
...
TASK: Draft a professional, warm renewal review email to a policyholder regarding policy ${ctx.policyNumber}, expiring on ${ctx.expirationDate || 'upcoming renewal date'}.

CLIENT & POLICY DETAILS:
${formatContextBlock(ctx)}

EMAIL PURPOSE:
Notify the client that their policy renewal is approaching. Explain the value of an annual review (preventing coverage lapses, closing coverage gaps, ensuring limits keep pace with construction costs). Ask permission to review coverage recommendations.

REFERENCE EMAIL BLUEPRINT (Match structure, tone, and flow):
---
Subject: Your Policy Renewal Is Approaching — Policy ${ctx.policyNumber}

Hi ${ctx.clientName ? ctx.clientName.trim().split(/\s+/)[0] : 'Valued Client'},

Did you know that an annual policy review could help you avoid a lapse in coverage, close potential coverage gaps, and ensure your limits keep pace with current construction costs? Since your policy (${ctx.policyNumber}) is coming up for renewal on ${ctx.expirationDate || 'your renewal date'}, now is a great time to review your coverage.
...
We won't take too much of your time. To speak with a licensed agent, please call our main office at (909) 626-5000${ctx.meetingUrl ? ` or schedule a review on our calendar: ${ctx.meetingUrl}` : ''}.
...
TASK: Draft a professional, warm, permission-based email requesting that policyholder ${ctx.clientName || 'Valued Client'} review and verify the property specifications on their attached Replacement Cost Estimate (RCE).
...
TASK: Draft a professional, warm renewal review email to a policyholder whose renewal premium is paid through mortgage escrow.

CLIENT & POLICY DETAILS:
${formatContextBlock(ctx)}

EMAIL PURPOSE:
Inform the client that their policy renewal is approaching. Clarify that while their mortgage escrow account (${ctx.mortgageeName || 'mortgage escrow'}) handles the premium payment, conducting an annual coverage review ensures their property remains fully protected against current rebuilding costs. Request permission to review potential coverage updates.
...
Your policy (${ctx.policyNumber}) is approaching its upcoming renewal on ${ctx.expirationDate || 'your renewal date'}. While your mortgage lender (${ctx.mortgageeName || 'your mortgage escrow account'}) handles premium payments, conducting an annual review ensures your coverage limits align with current rebuilding costs.
...
We respect your time and this quick review will ensure your renewal stays on track. Call our office at (909) 626-5000${ctx.meetingUrl ? ` or schedule a review on our calendar: ${ctx.meetingUrl}` : ''}.
...
EMAIL PURPOSE:
Invite the client to schedule a brief appointment to review their coverage options and discuss recommended policy updates. Reassure them that the review is quick, permission-based, and designed to protect their property investment. Include Calendly link ${ctx.meetingUrl || ''}.
...
TASK: Draft a professional, warm email to a policyholder.
